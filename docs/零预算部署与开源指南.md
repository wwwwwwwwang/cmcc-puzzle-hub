...
5. 进入 SQL Editor,依次粘贴执行 `supabase/migrations/` 下的 `0001`→`0002`→`0003`→`0004`→`0005`→`0006`→`0007`→`0008`(建表、RLS、plpgsql 函数、用户名审核、求助托管确认与 Cron、审核通过固定发放 3 点信用、用户管理封禁以及分页搜索)。
...
8. (可选)在 SQL Editor 配置信用上限 GUC;审核通过固定发放 3 点信用,这里只配置每日赚取封顶:
...
若变量无误,多半是 **Supabase 迁移未执行**(大厅或用户管理走 RPC,函数不存在即报错)。在 Supabase SQL Editor 跑 `select proname from pg_proc where proname in ('list_hall_posts', 'sync_request_maintenance', 'list_users');`,缺少任一函数都说明 `0001`→`0008` 未全部执行。
...
- [ ] Supabase 迁移 `0001`→`0008` 已全部执行，`pg_cron` 维护任务有效，邮箱确认已关闭，首个管理员已设置。
