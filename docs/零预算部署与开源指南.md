...
本文说明如何把“中国移动拼图互助平台”以 **0 元预算**部署到 Vercel，并使用 Upstash Redis 免费数据库。后半部分说明如何将仓库按 **GNU Affero General Public License v3.0（AGPL-3.0）**开源。

文档基于 2026-08-05 的官方页面和当前仓库状态编写。Vercel、Upstash 和 GitHub 可能调整控制台名称或免费额度，正式操作时应再次查看文末官方链接。

## 1. 最终架构

```mermaid
flowchart LR
  U[手机浏览器] --> V[Vercel Hobby<br/>Next.js 应用和 API]
  V --> R[Upstash Redis Free<br/>帖子、索引、限流和领取回执]
...
| Upstash Redis Free | Redis 数据库 | 一个 Free 数据库 |

不购买域名，不开通 Vercel Pro，不启用 Vercel Marketplace 付费集成，不把 Upstash 数据库升级到 Pay as You Go 或 Fixed。
...
本项目所有帖子和去重数据最多保存 24 小时，领取回执只保存 5 分钟，因此能够主动回收空间。实际是否够用取决于访问量，应定期查看 Upstash Metrics。
...
- Redis 写入带 24 小时 TTL，领取回执带 5 分钟 TTL
- 服务端使用 `@upstash/redis` 的 `Redis.fromEnv()`
...
建议 Vercel 和 Upstash 都使用 GitHub 登录，减少额外密码。为 GitHub 开启双重验证，并妥善保存恢复码。
...
```powershell
git grep -n -I -E -e 'UPSTASH_REDIS_REST_TOKEN=[A-Za-z0-9._-]{20,}' -e 'DEVICE_HASH_SECRET=[[:xdigit:]]{64}' -e 'ghp_[A-Za-z0-9]{20,}' -e 'vercel_[A-Za-z0-9._-]{20,}'
```
...
公开截图和 Issue 时，不要展示仍有效的真实领取口令、完整分享 URL、手机号、设备标识、Redis Token 或服务端日志中的敏感值。
...
不要使用只读 Token；本项目需要写入、删除和执行 Lua 脚本。不要把 Token 粘贴到聊天、Issue、README 或代码文件中。

## 11. 生成 DEVICE_HASH_SECRET

该密钥用于服务端对 FingerprintJS visitorId 做 HMAC-SHA256。它必须是随机值，且不能与 Upstash Token 相同。
...
```powershell
$deviceHashSecret = [Convert]::ToHexString($secretBytes).ToLowerInvariant()
```
...
最后一条命令会显示 64 位十六进制字符串。把它临时保存在本地密码管理器中，后续填入 Vercel。不要提交到 Git。

轮换该密钥会改变匿名设备哈希。轮换后的 24 小时内，旧帖子和新请求使用的设备哈希可能不一致，因此非泄露事故不要随意轮换。
...
DEVICE_HASH_SECRET=第11节生成的64位随机值
...
| `DEVICE_HASH_SECRET` | 64 位随机密钥 | 是 | Production |
| `PUBLISH_LIMIT_PER_HOUR` | `10`，访问量增大时可降到 `5` | 否 | Production |

第一轮只选 Production，避免未审查的 Preview 代码接触生产数据库。

如果以后需要自己的 Preview 部署读写 Redis，有两个选择：

1. 继续只给受信任分支授权，并把相同变量添加到 Preview。缺点是预览数据会进入生产数据库。
2. 使用独立测试数据库。隔离更安全，但可能超出一个免费数据库或免费额度的边界。
...
2. 打开发布页，确认 FingerprintJS 身份加载完成。
3. 选择拼图，分别验证口令、二维码和双来源发布。
4. 返回大厅，确认卡片显示正确来源。
5. 使用另一浏览器或另一台设备领取。
...
当前 visitorId 是匿名设备约束，不是认证凭证。攻击者可以伪造 visitorId 绕过按设备发布限制。免费方案出现滥用时：

1. 在 Vercel 查看 Functions 和 Firewall 日志。
2. 临时调低 `PUBLISH_LIMIT_PER_HOUR` 并重新部署。
3. 使用 Vercel Hobby 可用的 WAF/IP 规则做有限拦截。
4. 必要时暂时关闭项目或把生产部署保护起来。
5. 不要为了维持服务而直接升级付费方案，先确认流量来源。
...
然后到 Deployments → 当前部署 → Functions/Runtime Logs 查找 `SERVICE_UNAVAILABLE`。服务端日志有意不记录 visitorId、口令、完整 URL 或 Token。
...
确认数据库仍是 Free，而不是付费方案。Free 达到限制时应接受限流或等待下个月额度恢复，不要添加信用卡自动扩容。检查是否存在异常发布流量，并确认 TTL 正常生效。

### 环境变量泄露

1. 立即在 Upstash 重置或轮换 REST Token。
2. 在 Vercel 更新 Token 并重新部署。
3. 如 `DEVICE_HASH_SECRET` 泄露，生成新值并重新部署。
