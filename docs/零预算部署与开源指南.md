...
4. 进入 SQL Editor,依次粘贴执行 `supabase/migrations/` 下的 `0001`→`0002`→`0003`→`0004`(建表、RLS、plpgsql 函数、用户名与审核)。
5. 进入 Authentication → Providers/Settings,**关闭邮箱确认(Confirm email)**。本项目用合成邮箱,收不到确认邮件,不关会导致登录失败。
6. (可选)在 SQL Editor 配置信用上限 GUC;留空则用函数内默认(种子 1、每日封顶 5):
...
7. 注册第一个账号后,在 SQL Editor 手动把它设为管理员:
...
- 定期(或用 Cron)调用 `cleanup_expired_posts()` 清理过期帖子与去重行。
...
若变量无误,多半是 **Supabase 迁移未执行**(大厅走 `list_hall_posts` RPC,函数不存在即报错)。在 Supabase SQL Editor 跑 `select proname from pg_proc where proname = 'list_hall_posts';`,无结果说明 `0001`→`0004` 未全部执行。
...
确认数据库仍是 Free，而不是付费方案。Free 达到限制时应接受限流或等待下个月额度恢复，不要添加信用卡自动扩容。检查是否存在异常发布流量,并用 `cleanup_expired_posts()` 清理过期数据。
...
- [ ] Supabase 迁移 `0001`→`0004` 已全部执行,邮箱确认已关闭,首个管理员已设置。
