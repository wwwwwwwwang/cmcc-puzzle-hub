...
5. 进入 SQL Editor,依次粘贴执行 `supabase/migrations/` 下的 `0001`→`0002`→`0003`→`0004`→`0005`→`0006`(建表、RLS、plpgsql 函数、用户名审核、求助托管确认与 Cron,以及审核通过固定发放 3 点信用)。
...
若变量无误,多半是 **Supabase 迁移未执行**(大厅走 `list_hall_posts` RPC,函数不存在即报错)。在 Supabase SQL Editor 跑 `select proname from pg_proc where proname in ('list_hall_posts', 'sync_request_maintenance');`,缺少任一函数都说明 `0001`→`0006` 未全部执行。
...
- [ ] Supabase 迁移 `0001`→`0006` 已全部执行，`pg_cron` 维护任务有效，邮箱确认已关闭，首个管理员已设置。
