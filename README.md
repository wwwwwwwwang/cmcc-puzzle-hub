...
移动端优先的周三充值日拼图互助 H5。用户发布时至少提供口令或二维码链接中的一种，也可以同时保存两种来源；领取者可选择更适合自己的方式。帖子 24 小时后自动过期，首位领取者通过 Postgres 单事务原子下架。

数据真相源是 **Supabase Postgres**（账号、信用、帖子、领取），Upstash Redis 只做限流。采用「赠一领一」信用制：管理员审核通过后赠 1 点，领取一次消耗 1 点，发布的赠送帖被**他人**领取时给发布者 +1（求助帖不参与信用）。账号用**用户名 + 密码**注册，注册后进入待审核，管理员在 `/admin` 核对微信群昵称后通过才发信用、才能发布和领取。
...
- 信用制:领取 -1;赠送帖被**他人**领取给发布者 +1(受每日封顶);求助帖不参与信用。并发正确性与信用变动全靠 plpgsql `SECURITY DEFINER` 函数的单事务,不靠应用层。
- 反多账号(Sybil)靠**微信群成员身份 + 人工审核**,不靠设备指纹(已弃用);IP 仅做注册限流 + `/admin` 同 IP 标黄。

## 隐私边界

- 大厅接口只返回安全列(含发布者公开标识),**绝不返回 `payloads`**;`payloads` 只由领取事务返回给领取者、或「我的帖子/已领取」返回给本人。
...
帖子无 TTL 概念，而是带 `expires_at` 列；读取时统一过滤 `expires_at > now()`，`cleanup_expired_posts()` 供 Cron 定时置为过期并清理去重行。
