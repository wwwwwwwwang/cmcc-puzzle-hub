...
- 信用制:领取赠送 -1；赠送帖被**他人**领取给发布者 +1(受每日封顶；领取人与发布者注册 IP 相同则视为疑似同一人,由 `claim_post` 判定不加分)；发布求助时 -1 并托管，助力者在主动/自动确认后 +1；未收到时原帖未过期则重新开放，已过期或无人助力到期则退还托管信用。并发正确性与信用变动全靠 plpgsql `SECURITY DEFINER` 函数的单事务,不靠应用层。
